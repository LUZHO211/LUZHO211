...
还有一些更常用的例如JDK的数据库驱动管理。JDK里面定义了一个标准的数据库驱动的接口`java.sql.Driver`让各个数据库厂商（例如MySQL、PostgreSQL）去提供自己的实现，因为只有各个数据库厂商才知道自己的数据库。各个数据库厂商只需要遵循`java.sql.Driver`接口的规范来开发自己的驱动，然后用户在使用的时候只需要引入各个数据库厂商的jar包依赖即可使用对应的数据库。
...
再看看MySQL提供的实现类的jar包里面：就是标准的SPI方式提供的实现
