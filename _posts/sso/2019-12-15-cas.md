...
**步骤2：**由于用户在访问应用A的时候，CAS已经向浏览器写入了全局Cookie：`CASTGC=TGT-12345678`，所以应用B通过http://cas.local.com/login?service=http%3a%2f%2fweb.b.com跳转到CAS服务器时，浏览器会自动给cas.local.com域名携带上`CASTGC=TGT-12345678`这个Cookie给CAS服务器。CAS服务器发现请求中带有`CASTGC=TGT-12345678`这个Cookie，则会验证`TGT-12345678`是否有效（即是否能查到用户的Session），若CAS服务器验证TGT有效，则认为用户登录过了，不会再向用户返回登录表单页面，而是直接为用户生成一个用于访问应用B的ST票据。
